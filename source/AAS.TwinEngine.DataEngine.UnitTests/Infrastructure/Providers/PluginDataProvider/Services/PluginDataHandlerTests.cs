...
public class PluginDataHandlerTests
{
    ...
    public async Task GetDataForShellDescriptorsByAssetIdsAsync_WhenNullDeserialization_ThrowsResponseParsingException()
    {
        ...
        await Assert.ThrowsAsync<ResponseParsingException>(() =>
            _sut.GetDataForShellsByAssetIdsAsync(manifests, filter, null, null, CancellationToken.None));
    }
    ...
    public async Task GetDataForShellsByAssetIdsAsync_WithIdShort_PassesIdShort()
    {
        ...
        _pluginDataProvider
            .GetDataForShellDescriptorsByAssetIdsAsync(Arg.Any<IList<PluginRequestMetaData>>(), Arg.Is<string?>(v => v == null), Arg.Is<string?>(v => v == targetIdShort), Arg.Any<int?>(), Arg.Any<string?>(), Arg.Any<CancellationToken>())
            .Returns([httpResponse.Content]);
