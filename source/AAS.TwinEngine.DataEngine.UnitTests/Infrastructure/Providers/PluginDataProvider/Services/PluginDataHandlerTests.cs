...
public class PluginDataHandlerTests
{
    ...
    public async Task GetDataForAllShellDescriptorsAsync_ReturnsListWithHrefSet()
    {
        ...
        _pluginDataProvider
            .GetDataForAllShellDescriptorsAsync(null, null, null, null, Arg.Any<IList<PluginRequestMetaData>>(), Arg.Any<CancellationToken>())
            .Returns([json]);

        var result = await _sut.GetDataForAllShellDescriptorsAsync(null, null, null, null, manifests, CancellationToken.None);
    ...
    public async Task GetDataForAllShellDescriptorsAsync_Throws_WhenDeserializationFails()
    {
        ...
        await Assert.ThrowsAsync<ResponseParsingException>(() =>
            _sut.GetDataForAllShellDescriptorsAsync(null, null, null, null, manifests, CancellationToken.None));
    }
