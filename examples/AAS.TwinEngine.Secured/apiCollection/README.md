...
| `KEYCLOAK_URL`                                | Keycloak base URL                                              | `http://localhost:9090`                                            |
| `REALM`                                       | Keycloak realm                                                 | `basyx`                                                            |
| `KEYCLOAK_GRANT_TYPE`                         | OAuth2 grant used for token acquisition                        | `password`                                                         |
| `CLIENT_ID`                                   | Keycloak client ID                                             | `basyx-ui`                                                         |
| `CLIENT_SECRET`                               | Secret for confidential clients                                | empty for `basyx-ui`                                               |
| `USERNAME`                                    | Keycloak user for password grant                               | `usera`                                                            |
| `PASSWORD`                                    | Keycloak password for password grant                           | `pwd`                                                              |
...
The collection pre-request script obtains a Keycloak token before the first secured request, stores it in `access_token`, and reuses it until 30 seconds before expiry. Existing requests use inherited collection authentication and therefore send `Authorization: Bearer {{access_token}}` automatically. Use `client_credentials` with a confidential client and `CLIENT_SECRET` when its service account has the required BaSyx permissions.
